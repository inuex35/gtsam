...
CarrierPhaseDDFactor::CarrierPhaseDDFactor(
    ...
    const SharedNoiseModel& model)
    : Base(model, positionKey, ambTargetKey, ambRefKey, ionoKey),
      sdPhiTarget_(sdPhiTarget), sdPhiRef_(sdPhiRef),
      satPosTarget_(satPosTarget), satPosRef_(satPosRef),
      basePos_(basePosition), ionoCoeff_(ionosphereCoefficient) {}
...
Vector CarrierPhaseDDFactor::evaluateError(
    ...
    OptionalMatrixType HambRef, OptionalMatrixType Hiono) const {
  // SD residuals:
  // sd_resid = (|pos - sat| - |base - sat|) + N + ionoCoeff*iono - sd_phi
  ...
  // DD = SD_target - SD_ref + ionoCoeff * dd_iono
  const double error = sd_resid_target - sd_resid_ref + ionoCoeff_ * ddIono;
